...
exports.getAllBalances = async (year, filters = {}) => {
  ...
  query += ` AND NOT EXISTS (
    SELECT 1 FROM user_roles ur_ex
    JOIN roles r_ex ON r_ex.id = ur_ex.role_id
    WHERE ur_ex.user_id = u.id AND ur_ex.status = 'active' AND LOWER(r_ex.role_name) = 'super admin'
  )`;

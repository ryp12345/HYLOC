...
export default function UnitMaster() {
  ...
  const load = async () => {
    try {
      const res = await getAllUnitMasters();
  ...
  const openEdit = (row) => {
    setEditingId(row.id);
    setForm({ unit_name: row.unit_name || '', symbol: row.symbol || '' });
    setIsModalOpen(true);
  };
  ...
  const exportToExcel = () => {
    if (!filtered.length) {
      showNotification('No units available to export', 'error');
      return;
    }

    const headers = ['S.NO', 'Unit Name', 'Symbol'];
    const rowsToExport = filtered.map((row, index) => [
      index + 1,
      `"${String(row.unit_name || '').replace(/"/g, '""')}"`,
    ...
    showNotification('Units exported successfully!', 'success');
  };
  ...
  const filtered = useMemo(() => {
    ...
    return sorted.filter(r => (
      r.unit_name?.toLowerCase().includes(q) || r.symbol?.toLowerCase().includes(q)
    ));
  }, [rows, search]);
    ...
    <div className="min-h-screen bg-[color:var(--app-bg)] px-4 py-12 text-[color:var(--text-primary)] transition-colors duration-300 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-12 text-center">
          <h1 className="mb-2 text-4xl font-extrabold text-[color:var(--text-primary)]">Unit Master</h1>
          <p className="text-lg text-[color:var(--text-secondary)]">Create, update and manage units</p>
        </div>

        <div className="flex flex-col items-start justify-between gap-4 mb-6 sm:flex-row sm:items-center">
          <div className="relative w-full sm:w-72">
            <input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Search units..." className="w-full rounded-lg border border-[color:var(--border)] bg-[color:var(--surface)] py-2 pl-10 pr-4 text-[color:var(--text-primary)] focus:border-[color:var(--accent)] focus:ring-2 focus:ring-[color:var(--focus-ring)]" />
            ...
            <table className="min-w-full divide-y divide-[color:var(--border)]">
              <thead className="bg-[color:var(--accent)]">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-xs font-medium uppercase tracking-wider text-white">Unit Name</th>
                  <th className="px-6 py-4 text-left text-xs font-medium uppercase tracking-wider text-white">Symbol</th>
              ...
              <tbody className="divide-y divide-[color:var(--border)] bg-[color:var(--surface)]">
                {filtered.length === 0 ? (
                  <tr><td colSpan="4" className="px-6 py-12 text-center text-[color:var(--text-muted)]">No units found</td></tr>
                ) : (
                  paginated.map((row, idx) => (
                    <tr key={row.id} className={`transition-colors duration-150 ${idx % 2 === 0 ? 'bg-[color:var(--surface)]' : 'bg-[color:var(--surface-hover)]'} hover:bg-[color:var(--surface-hover)]`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-[color:var(--text-primary)]">{row.unit_name}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-[color:var(--text-primary)]">{row.symbol}</td>
              ...
              <div className="inline-block overflow-hidden text-left align-bottom transition-all transform rounded-lg border border-[color:var(--border)] bg-[color:var(--surface)] shadow-xl sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
                <div className="bg-[color:var(--accent)] px-6 py-4">
                  <div className="flex items-center justify-between">
                    <h3 className="text-lg font-medium leading-6 text-white">{editingId ? 'Edit Unit' : 'Add Unit'}</h3>
                ...
                <div className="bg-[color:var(--surface)] px-6 py-5">
                  ...
                  <form className="space-y-5" onSubmit={submit}>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="mb-2 block text-sm font-medium text-[color:var(--text-secondary)]">Unit Name *</label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="mb-2 block text-sm font-medium text-[color:var(--text-secondary)]">Symbol</label>
                        <input
                          type="text"
                          value={form.symbol}
                    ...
                    <div className="flex justify-end space-x-4 pt-4">
                      <button type="button" onClick={onClose} className="inline-flex justify-center rounded-lg border border-[color:var(--border)] bg-[color:var(--surface)] px-6 py-3 text-sm font-medium text-[color:var(--text-secondary)] shadow-sm hover:bg-[color:var(--surface-hover)]">Cancel</button>
                      <button type="submit" className="inline-flex justify-center rounded-lg bg-[color:var(--accent)] px-6 py-3 text-sm font-medium text-white shadow-sm hover:opacity-90">{editingId ? 'Update Unit' : 'Add Unit'}</button>
                    </div>
                  </form>
                </div>
              </div>

...
export default function AssociationsPage() {
  ...
  const exportToExcel = () => {
    if (!filtered.length) {
      showNotification('No associations available to export', 'error');
      return;
    }

    const headers = ['S.NO', 'Association Name', 'Status'];
    const rowsToExport = filtered.map((row, index) => [
      index + 1,
      `"${String(row.association_name || '').replace(/"/g, '""')}"`,
    ...
    link.href = url;
    link.setAttribute('download', `associations_${new Date().toISOString().slice(0, 10)}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    window.URL.revokeObjectURL(url);

    showNotification('Associations exported successfully!', 'success');
  };

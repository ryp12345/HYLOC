                    ...
                    <tr key={e.user_id+e.year} className={`${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-blue-50 transition-colors duration-150`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                        <div className="flex items-center justify-center space-x-2">
                          <button onClick={() => { setSelected(e); setModal('edit'); }} className="p-2 text-white transition-colors duration-200 bg-blue-600 rounded-lg hover:bg-blue-700" title="Edit Entitlement">
                          ...
                          <button onClick={async () => { if (window.confirm('Are you sure you want to delete this entitlement?')) { try { await deleteEntitlement(e.id, token); await loadEntitlementData(); } catch (err) { setError(err.response?.data?.error || 'Failed to delete entitlement'); } } }} className="p-2 text-white transition-colors duration-200 bg-red-600 rounded-lg hover:bg-red-700" title="Delete Entitlement">
